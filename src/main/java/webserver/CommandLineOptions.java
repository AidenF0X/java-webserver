...
import static webserver.App.cfo;
...
public class CommandLineOptions {
	...
	private OptionSpec<Integer> withPort() {
		return parser.acceptsAll(Arrays.asList("port", "p"),
                        "Port to bind the web server to. Can be any unused port in the range 0-65535").withRequiredArg().ofType(Integer.class).defaultsTo(DEFAULT_PORT);
	}
	...
	private OptionSpec<String> withDocroot() {
		return parser.acceptsAll(Arrays.asList("docroot", "d"),"Path to the document root containing the files the server will serve").withRequiredArg().defaultsTo(DEFAULT_DOCROOT);
	}
	...
	private OptionSpec<Integer> withTimeout() {
		return parser.acceptsAll(Arrays.asList("timeout", "t"),"Request timeout (in seconds). Requests which take longer than this will be terminated").withRequiredArg().ofType(Integer.class).defaultsTo(DEFAULT_TIMEOUT);
	}
	...
	private OptionSpec<Integer> withMaxThreads() {
		return parser.acceptsAll(Arrays.asList("maxthreads", "m"),"Maximum number of worker threads that may be spawned to service client requests").withRequiredArg().ofType(Integer.class).defaultsTo(DEFAULT_MAX_THREADS);
	}
	...
	private OptionSpec<Void> withHelp() {
		return parser.acceptsAll(Arrays.asList("help", "h"),"Display help/usage information").forHelp();
	}
